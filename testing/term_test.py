...
class Terms(unittest.TestCase):
    ...
    def runTest(self):
        ...
        def test(driver, skip):
            ...
            driver.find_elements_by_css_selector("#newModal #NewTerm_Name")[
                0
            ].send_keys("test term")
            time.sleep(1)
            ...
            action.click(
                on_element=driver.find_elements_by_css_selector(
                    'div[data-inputid="NewTerm_Summary"] .CodeMirror'
                )[0]
            ).perform()
            action.send_keys(
                "# very nice summary"
            ...
            action.click(
                on_element=driver.find_elements_by_css_selector(
                    'div[data-inputid="NewTerm_TechnicalDefinition"] .CodeMirror'
                )[0]
            ).perform()
            action.send_keys(
                "# very nice summary"
            ...
            # delete
            driver.find_elements_by_css_selector(
                'a.nav-link[data-tooltip="Delete this Term"]'
            )[0].click()
            time.sleep(1)
