...
function processAdd(cmd) {
  ...
  if (ct) {
    ...
    if (doc_type) {
      doc.setField('doc_type', doc_type);
    }
  ...
  var token_stream = analyzer.tokenStream('content', content);
  var term_att = token_stream.getAttribute(
    get_class('org.apache.lucene.analysis.tokenattributes.CharTermAttribute'),
  );
  var type_att = token_stream.getAttribute(
    get_class('org.apache.lucene.analysis.tokenattributes.TypeAttribute'),
  );
  token_stream.reset();
  while (token_stream.incrementToken()) {
    doc.addField(
      type_att.type().replace(/\<|\>/g, '').toLowerCase() + '_ss',
      term_att.toString(),
    );
  }
  token_stream.end();
