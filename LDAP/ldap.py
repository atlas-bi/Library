...
c = Connection(
    s, settings.username, settings.password, auto_bind=True, auto_referrals=False
)
c.start_tls()
...
for base in settings.search_bases:
    ...
    generator = c.extend.standard.paged_search(
    ...
    for q in generator:
        ...
        row = [
            ...
            settings.ad_domain + "\\" + data["sAMAccountName"]
            ...
            data["title"]
            if "title" in data
            else "" or data["description"][0]
            if "description" in data
            else "",  # department
            data["ipPhone"]
            if "ipPhone" in data
            else "" or data["telephoneNumber"]
            if "telephoneNumber" in data
            else "",  # phone number
            data["mail"]
            if "mail" in data
            else "" or data["proxyAddresses"]
            if "proxyAddresses" in data
            else "" or data["userPrincipalName"]
