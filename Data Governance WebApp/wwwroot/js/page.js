...
(function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.site-messageClose')) {
            document.querySelector('.site-message').parentElement.removeChild(document.querySelector('.site-message'));
            if (title) {
    ...
    if (title) {
        ...
        siblingPadding = parseInt(window.getComputedStyle(sibling, null).getPropertyValue('padding-top'), 10);
    }
    ...
    var scrollHead = function() {
        ...
        if (title && ((pageHeight - window.innerHeight) > sticky * 2 || (pageHeight - window.innerHeight) <= sticky)) {
    ...
    function relatedReports() {
        ...
        if (ids.length > 0) {
            ...
            div.setAttribute('data-url', '/Reports/?handler=RelatedReports&id=' + (ids.slice(0, 5)).join(','));
