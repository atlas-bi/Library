...
var d = document,
  ...
  l = debounce(function() {
    ...
    setTimeout(function() {
      ...
      for(var el=0;el<dest.length;el++){
        ...
        if(dest[el].getElementsByClassName('chart').length > 0) { dest[el].removeChild(dest[el].getElementsByClassName('chart')[0]);}
        if(dest[el].getElementsByClassName('ajaxLoader').length > 0) { dest[el].removeChild(dest[el].getElementsByClassName('ajaxLoader')[0]);}
  ...
  k = d.requestAnimationFrame || d.setImmediate || function (b) {
  ...
  build = function (json, w) {
    var chart = JSON.parse(json),
      ...
      html = '',
      cWidth =  w || 800,
      c,q,thisChart,style,side,axii = [],cols=0,range={},a,bars=0,colWidth,
      numbers = ['one','two','three','four','five'];

      this.table = document.createElement('table');

      for (c in charts){ if (axii.indexOf(charts[c].axis) == -1) {axii.push(charts[c].axis);}}
      ...
      this.table.innerHTML += '<caption class="chart-caption">' + chart.title + '</caption>';

      if ((!!options && options.hasOwnproperty('legend') && options.legend !== 'false') || !options) {
        this.thead = document.createElement('thead');
        this.thead.classList.add('chart-head');
        var th;
        for (c in charts) {
          th = document.createElement('th');
          th.innerHTML += '<span class="chart-headClr '+numbers[c]+'"></span>' + charts[c].title;
      ...
      for (c in charts){ cols = Math.max(cols, charts[c].data.length);}
      ...
      for (var d = 0; d < cols; d++) {
        ...
        for(c in charts){
          tooltip += charts[c].data[d].data+' '+axis[charts[c].axis].title + '; ';
        }
        ...
        for (c in charts) {
          ...
          var lheight = parseFloat(thisChart.data[d].data) == 0 ? 0 : parseFloat(thisChart.data[d].data) / thisChart.range * 100;
          maxHeight = Math.max(maxHeight,lheight);
          var lwidth = "calc(" + 100 / bars + '% - 5px)';

          if (c == 0) html += '<th class="chart-xAxisTitle" scope="row" style="left:'+(-26+((colWidth / 100 * width)/2) - 15)+'px">' + thisChart.data[d].title;

          var p = lheight / 100 * height < 30 ? 'style="margin-top:-22px"' : "";
          if (thisChart.type == 'bar') {
            html += '<td class="chart-' + thisChart.type + ' ' +numbers[c]+'" style="height:' + lheight + '%;width:' + lwidth + ';left:' + barnum / bars * 100 + '%"><p ' + p + '></p></td>';
          }
          else if (thisChart.type == 'line') {
            html += '<td class="chart-' + thisChart.type + ' ' +numbers[c] + '" style="height:' + lheight + '%">';
            html += '<p ' + p + '></p>';
            if (!!thisChart.lastlinePointY || thisChart.lastlinePointY == 0) {
              w = ((colWidth / 100 * width));
              var angle = Math.atan2(lheight / 100 * height - thisChart.lastlinePointY, -w) * 180 / Math.PI;
              var distance = Math.sqrt(Math.pow((w), 2) + Math.pow((lheight / 100 * height - thisChart.lastlinePointY), 2));
        ...
        html += '<td class="chart-tooltip" data-tooltip="' + tooltip+'" style="height:calc('+maxHeight+'% + 20px)"></td>';
        html += '</tr>';
      }
      ...
      for (c in axii) {
        ...
        if (typeof axis[a] != 'undefined') {
          html += '<div class="chart-yAxisTitle ' + side + '" style="' + side + ':-' + style + 'px"><p>' + axis[c].title + '</p></div>';
        }
      }
      ...
      for (c = 1; c <= ticks; c++) {
        html += '<div class="chart-ticksTick' + (c==1 ? ' first ' : '') + (c==ticks ? ' last ' : '') +'">';
        for (q in range) {
          style = (Math.floor(q / 2) + 1) * (q % 2 == 1 ? 30 : 30);
          side = q % 2 == 1 ? 'right' : 'left';
          if(range[q] > 0){
            html += '<p style="' + side + ':-' + style + 'px">' + Math.round(range[q] - ((range[q] / ticks)*(c-1))) + '</p>';
          }
        }
        html += '</div>';
      }
  ...
  d.addEventListener('load-charts',function(){l();});
  d.addEventListener('mdl-open',function(){l();});
  d.addEventListener('tab-opened',function(){l();});
  d.addEventListener('collapse-opened',function(){l();});
