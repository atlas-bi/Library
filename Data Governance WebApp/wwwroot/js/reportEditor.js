...
(function() {
    ...
    /* details */
    d.getElementById('report-edit-details-form').addEventListener('submit', function(e) {
        e.preventDefault();
        var form = e.target,
        ...
        q.onload = function() {
            ...
            setTimeout(function() {
                d.getElementById('editorMdl-titleSave').style.removeProperty("visibility");
            }, 750);
        };
    });

    // maintenance
    d.getElementById('report-edit-new-maintenance').addEventListener('submit', function(e) {
        e.preventDefault();
        var form = e.target,
            q = new XMLHttpRequest();
        q.open('post', form.getAttribute('action') + '&' + serialize(form), true);
    ...
    d.querySelector('#editModal').addEventListener('submit', function(e) {
        if (e.target.closest('#report-edit-new-meticket') || e.target.closest('#report-editor-remove-meticket-form')) {
            ...
            q.send();
            q.onload = function() {
                d.getElementById('report-editor-metickets-container').innerHTML = q.responseText;
                form.reset();
            };
        }
    });
    ...
    d.querySelector('#editModal').addEventListener('change', function(e) {
        ...
        if (e.target.closest('#NewTermLink_TermId')) {
            var t = e.target.closest('#NewTermLink_TermId'),
                q = new XMLHttpRequest();
            q.open('post', "/Reports?handler=CurrentTermDetails&TermId=" + t.getAttribute('value'), true);
            q.setRequestHeader('Content-Type', 'text/plain;charset=UTF-8`');
            q.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
            q.send();
            q.onload = function() {
                if (q.responseText) {
                    ...
                    d.getElementsByClassName('new-term-summary')[0].innerHTML = '<div class="markdown noleft">' + data.summary || 'n/a' + '</div>';
                    d.getElementsByClassName('new-term-def')[0].innerHTML = '<div class="markdown noleft">' + data.technicalDefinition || 'n/a' + '</div>';
                    // render markdown
