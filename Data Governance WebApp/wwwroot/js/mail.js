...
(function() {
    ...
    var mail_check_rate = 10 * 1000,
        ...
        parse_new_mail_data = function(e) {
            ...
            if (d.querySelector('#mlbx-fldrsDrafts .mail-draft-cnt')) {
                d.querySelector('#mlbx-fldrsDrafts .mail-draft-cnt').innerHTML = draftMsg > 0 ? draftMsg : '';
            }
            ...
            if (not.innerHTML.length > 0) {
                var notBox = document.getElementsByClassName('mail-notification-container')[0];
            ...
            if (d.querySelector('#mlbx-fldrsInbox.active') !== null) {
                ...
                if (div.innerHTML.length > 0) {
                    ...
                    var msg = Array.prototype.slice.call(d.querySelectorAll('.mlbx-msgsMsg[data-rep-id]')),
                        el;
        ...
        LoadFolder = function(folder, folderId) {
            ...
            if (folder !== null) loadMesQ.open('get', "mail?handler=Messages&folder=" + folder, true);
            if (typeof folderId !== 'undefined' && folderId !== null) loadMesQ.open('get', "mail?handler=Messages?folderId=" + folderId, true);
            ...
            loadMesQ.onload = function() {
                ...
                // load first message
                if (folder == "inbox" || folder == "deleted" || folder == "sent" || (typeof folderId !== 'undefined' && folderId !== null)) {
                    if (d.querySelector('.mlbx-msgs .mlbx-msgsMsg.active')) LoadMessageDetails(d.querySelector('.mlbx-msgs .mlbx-msgsMsg.active').getAttribute('data-rep-id'));
                } else if (folder == "drafts") {
                    if (d.querySelector('.mlbx-msgs .mlbx-msgsMsg.active')) {
                        LoadDraft(d.querySelector('.mlbx-msgs .mlbx-msgsMsg.active').getAttribute('data-draft-id'));
                    } else {
                        StartNewMessage();
                    }
                }
        ...
        SaveDraft = function(form) {
            ...
            if (saveDraftQ === null || form.querySelector('input[name="draftId')) {
                ...
                var data = {
                    ...
                    Text: form.querySelector('.mlbx-newMsgMsgBx').innerText.replace(/\n/g, " "),
                    MsgId: null,
                    ConvId: null
                };
                if (form.querySelector('input[name="draftId')) {
                    data.DraftId = form.querySelector('input[name="draftId').getAttribute('value');
                }

                saveDraftQ = new XMLHttpRequest();
                saveDraftQ.open('post', "mail?handler=SaveDraft", true);
                saveDraftQ.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
        ...
        LoadDraft = function(id) {
            ...
            loadMesQ.onload = function() {
                ...
                d.getElementsByClassName('mlbx-rdr')[0].innerHTML = div.getElementsByClassName('mlbx-newMsg')[0].outerHTML;
                ...
                d.querySelector('.mlbx-newMsg form.mlbx-newMsgForm').addEventListener('input', function(e) {
                    SaveDraft(this);
                });
                d.querySelector('.mlbx-newMsg form.mlbx-newMsgForm').addEventListener('change', function(e) {
                    SaveDraft(this);
                });
            };
        },
        ...
        StartNewMessage = function(to, subject, message, convid) {
            ...
            if (d.querySelector('.mlbx-msgsMsg.active')) d.querySelector('.mlbx-msgsMsg.active').classList.remove('active');
            var messagebox = d.getElementsByClassName('mlbx-rdr')[0];
            messagebox.innerHTML = '';
            messagebox.appendChild(d.querySelector('.new_message_template .mlbx-newMsg').cloneNode(true));

            messagebox.querySelector('form.mlbx-newMsgForm').addEventListener('input', function(e) {
                SaveDraft(this);
            });
            messagebox.querySelector('form.mlbx-newMsgForm').addEventListener('change', function(e) {
                SaveDraft(this);
            });
            ...
            if (typeof to !== 'undefined' && to != null) {
                ...
                to.forEach(function(t) {
                    dds.innerHTML += '<option selected="selected" class="' + t[2] + '" value="' + t[0] + '">' + t[1] + '</option>';
                    ddw.children[0].insertAdjacentHTML('beforebegin', '<div class="dd-itm ' + t[2] + '">' + t[1] + '</div>');
        ...
        LoadMessageDetails = function(id) {
            ...
            loadMesQ.send();
            loadMesQ.onload = function() {
                if (d.getElementsByClassName('mlbx-rdr')[0]) d.getElementsByClassName('mlbx-rdr')[0].innerHTML = loadMesQ.responseText;
                document.dispatchEvent(new CustomEvent('ss-load'));
            };
        },
        ...
        ChangeActiveMessage = function(me) {
            ...
            if (d.querySelector('#mlbx-fldrsInbox.active') && me.classList.contains('unread')) {
    ...
    d.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.mlbx-msgsMsg')) {
            ...
            var from = e.target.closest('.mail-new-message').getAttribute('data-from');
            var fromId = e.target.closest('.mail-new-message').getAttribute('data-fromid');
            if (from && fromId) {
                StartNewMessage([
                    [fromid, from]
                ]);
            ...
            // user dropdown
            e.target.closest('.mlbx-usrDrp').querySelector('.mlbx-usrDrpCtr').style.display = 'block';
            e.target.closest('.mlbx-usrDrp').querySelector('.mlbx-usrDrpCtr').addEventListener('mouseleave', function mouseleave(e) {
                this.style.removeProperty('display');
                this.removeEventListener('mouseleave', mouseleave, false);
            });
        } else if (e.target.closest('#mlbx-fldrsDrafts')) {
            LoadFolder('drafts');
            document.querySelector('.mlbx-fldrsFldr.active').classList.remove('active');
            ...
            if (msg.hasAttribute('data-draft-id')) {
                ...
                deleteMes.open('post', "mail?handler=DeleteDraft&id=" + msg.getAttribute('data-draft-id'), true);
                deleteMes.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
                ...
                // delete message
                deleteMes = new XMLHttpRequest();
                deleteMes.open('post', "mail?handler=DeleteMessage&id=" + msg.getAttribute('data-rep-id'), true);
                deleteMes.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
                deleteMes.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
                deleteMes.send();
            ...
            msg.style.transition = "margin-left 150ms, margin-right 150ms, margin-top 150ms";
            msg.style.zIndex = '-1';
    ...
    d.addEventListener('submit', function(e) {
        if (e.target.closest('.mlbx-newMsgForm')) {
            ...
            if (form.querySelector('input[name="draftId"]') !== null) {
                ...
                if (d.querySelector('.mlbx-msgsMsg[data-draft-id="' + draftId + '"]')) {
                    d.querySelector('.mlbx-msgsMsg[data-draft-id="' + draftId + '"]').parentElement.removeChild(d.querySelector('.mlbx-msgsMsg[data-draft-id="' + draftId + '"]'));
                }
            }
            ...
            sendMes.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded; charset=UTF-8');
    ...
    d.addEventListener('click', function(e) {
        ...
        if (e.target.closest('.mail-reply') && d.querySelector('#mlbx-fldrsInbox.active') !== null) {
            ...
            message_liner.innerHTML = d.querySelector('.mlbx-rdr .ss-content').innerHTML;
            message_liner.querySelector('.mlbx-rdrHead-controls').parentElement.removeChild(message_liner.querySelector('.mlbx-rdrHead-controls'));
        ...
        } else if (e.target.closest('.mail-reply-all') && d.querySelector('#mlbx-fldrsInbox.active') !== null) {
            m = d.querySelector('.mlbx-rdrHead');
            convid = m.getAttribute('data-conv-id');
            subject = "Re: " + m.querySelector('.mailbox-subject').innerText;
            ...
            for (x = 0; x < f.length; x++) {
                from.push([f[x].getAttribute('data-user-id'), f[x].querySelector('span').innerText, f[x].classList.contains('group') ? "group" : ""]);
            }
            for (x = 0; x < t.length; x++) {
                from.push([t[x].getAttribute('data-user-id'), t[x].querySelector('span').innerText, t[x].classList.contains('group') ? "group" : ""]);
            }
            // remove dups
