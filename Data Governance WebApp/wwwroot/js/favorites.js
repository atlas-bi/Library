...
(function (root, factory) {
  ...
  var x = function(){
    ...
    d.addEventListener('click', function (e) {
      if (e.target.closest('.favorite:not(.disabled)') || e.target.closest('.favorite-search:not(.disabled)')  || e.target.closest('.fav-star:not(.disabled)')) {
        e.preventDefault();
        e.stopPropagation();
        var t = e.target.closest('.favorite-search:not(.disabled)') ? e.target.getElementsByTagName('i')[0] : e.target,
            x,
            el,
            q,
            data,
            ...
            hasFavBox = document.getElementsByClassName('favs')[0] == null ? false : true,
            actionType = 1,
            favoriteType = t.getAttribute("fav-type"),
            objectId = t.getAttribute("object-id"),
            objectName = t.getAttribute("object-name"),
            l = d.querySelectorAll('[fav-type="' + favoriteType + '"][object-id="' + objectId + '"]');
        ...
        q.onload = function () {
          if (hasFavBox) {
            document.dispatchEvent(new CustomEvent('reload-favs'));
          }
        };
      }
    }, false);
    ...
    function showall(me) {
      ...
      if (folderId !== null) {
        i = d.querySelectorAll('.favs div[folder-id]:not([folder-id="' + folderId + '"])');
      ...
      if (folderId !== null && d.querySelectorAll('.favs div[folder-id="' + folderId + '"]').length == 0 || folderId == null && d.querySelectorAll('.favs div[folder-id]').length == 0) {
        i = nr.childNodes;
    ...
    d.addEventListener('submit', function (e) {
      ...
      if (e.target.closest('#CreateFolderForm')) {
        ...
        q.onload = function () {
          ...
          div.innerHTML = '<i class="fas fa-folder mr-3"></i>' + v + '<div class="folder-grip"><i class="fas fa-grip-lines fa-folder"></i></div>';
          var nf = f.getElementsByClassName('fav-folder-new')[0];
          nf.parentElement.insertBefore(div, nf);
          s.innerHTML = '';
          document.dispatchEvent(new CustomEvent('clps-close', {
            cancelable: true,
            detail: {
        ...
        q.onload = function () {
          var m = d.getElementById('fav-folders').querySelector("div[folder-id][folder-id='" + folderId + "']");
    ...
    function Reorder(el, x, y) {
      ...
      if (el.classList.contains('fav-folder')) {
        e = el.parentElement.querySelectorAll('.fav-folder:not(.fav-folder-new):not(.fav-show-all');
    ...
    d.addEventListener('dragMove', function (e) {
      if (typeof e.detail !== 'undefined') {
        var i = d.querySelectorAll('#fav-folders .fav-folder:not(.fav-folder-new)'),
            l,
            el;
    ...
    function UpdateFolderRank() {
      var array = [],
          g,
          s = document.querySelectorAll('#fav-folders .fav-folder:not(.drag-source)'),
          q;
    ...
    function UpdateFavRank() {
      var array = [],
          g,
          s = document.querySelectorAll('.favs div[folder-id]:not(.drag-source)'),
          q;
